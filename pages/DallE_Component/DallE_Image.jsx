...
export default function DallE_Image() {
    ...
    const getpromptRef = useRef();
    const sendPrompt = async () => {
        
        setImage(null);
        if(!prompt) return;
        setloader(true);
        const input = getpromptRef.current.value;
      console.log(input)
      ...
      const shareToCommunity = async () => {
        const docRef = await addDoc(collection(db, "CommunutyImages"),{
          Image_Url: image,
  ...
  return (
    ...
    <div class="relative mx-[5rem] lg:mx-96 md:mx-56">
        ...
        <input type="search" id="search" class="block w-full p-4 pl-10 text-sm text-white border border-gray-300 rounded-lg bg-transparent placeholder:text-slate-500 focus:bg-transparent" placeholder="Enter Prompt" spellCheck="false"
            autoCorrect="false" ref={getpromptRef}/>
...
{session? (
  <>
  {image ? (
    <a class="relative inline-flex items-center justify-start py-3 pl-4 pr-12 overflow-hidden font-semibold text-indigo-600 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 bg-gray-50 group cursor-pointer"
